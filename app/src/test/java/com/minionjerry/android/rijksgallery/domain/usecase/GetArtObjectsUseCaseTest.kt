...
class GetArtObjectsUseCaseTest {
    ...
    fun testProcess() = runTest {
        ...
        val pagingData = PagingData.from(listOf(artObject1,artObject2,artObject3))

        whenever(artObjectRepository.getArtObjects()).thenReturn(flowOf(pagingData))

        val response = useCase.process(GetArtObjectsUseCase.Request).first()
        assertEquals(GetArtObjectsUseCase.Response(pagingData), response)
    }
}